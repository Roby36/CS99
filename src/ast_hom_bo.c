...
int main(int argc, char *argv[]) {
    ...
    if (argc != 8) { 
        fprintf(stderr, "Usage: %s <input_json_path> <table_title> <bound> <var_mult> <MAX_IT> <max_expandible_states_mult> <max_hom_classes> \n", argv[0]);
        return 1; 
    }

    PARSE_STRING(input_json_path, argv[1], "<input_json_path>");
    PARSE_STRING(table_title, argv[2], "<table_title>");
    PARSE_FLOAT(argv[3], bound);
    PARSE_FLOAT(argv[4], var_mult);
    PARSE_INT(argv[5], MAX_IT);
    PARSE_INT(argv[6], max_expandible_states_mult);
    PARSE_INT(argv[7], max_hom_classes);
